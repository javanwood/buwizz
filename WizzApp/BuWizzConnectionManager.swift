...
class BuWizzConnectionManager: NSObject {
    ...
    fileprivate var targetBuWizzIds: [UUID] = []
    fileprivate var didConnectBuWizz: ((BuWizz) -> ())?
    ...
    func startScan(forBuWizzIds buWizzIds: [UUID], didConnectBuWizz: @escaping (BuWizz) -> ()) {
        if centralManager.state != .poweredOn {
            return
        }
        self.didConnectBuWizz = didConnectBuWizz
        centralManager.scanForPeripherals(withServices: [motorControlServiceUUID], options: nil)
    }
...
extension BuWizzConnectionManager: CBCentralManagerDelegate {
    ...
    func centralManager(_ central: CBCentralManager, didDiscover peripheral: CBPeripheral, advertisementData: [String : Any], rssi RSSI: NSNumber) {
        // Make sure it's a target peripheral
        if targetBuWizzIds.contains(peripheral.identifier) {
            return
        }
